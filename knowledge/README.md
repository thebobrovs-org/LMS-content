...
terms: [bandwidth-bound, ridge]           # optional, ≤ 20: words a question may use for this record; a concept's **Canonical terms.** line joins them in the index (LMS-content#111)
...
`knowledge/index.json` is generated by `node pipeline/knowledge-index.mjs` from the **approved** records only: ids, types, titles, scopes, tags, terms (the `terms` list and a concept's canonical terms), what they touch, their sources and review dates, and the links between them both ways; and `simulations`, each simulation with the published lessons that embed it. Retrieval by lesson (`pipeline/knowledge-search.mjs --topic <id>`, and the mentor's ranking) reaches a record through every reference form: the topic, its objectives, items and steps, and the simulations and checkpoints the lesson embeds. The gate checks it is current (`--check`); regenerate and commit it with any record change. A `proposed` record is visible here, in the repository, as any PR is; it never reaches the index or the app.
