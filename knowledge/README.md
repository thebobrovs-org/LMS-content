...
`knowledge/index.json` is generated by `node pipeline/knowledge-index.mjs` from the **approved** records only: ids, types, titles, scopes, tags, terms (the `terms` list and a concept's canonical terms), what they touch, their sources and review dates, and the links between them both ways. The gate checks it is current (`--check`); regenerate and commit it with any record change. A `proposed` record is visible here, in the repository, as any PR is; it never reaches the index or the app.
