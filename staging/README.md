...
- Promoted with `node pipeline/promote.mjs staging/topics/<…>.mdx`. It validates
  first, then moves the file to `topics/` and sets `status: published`. Only files
  in `staging/topics/` are accepted, and a published topic is replaced only with
  `--force`.
